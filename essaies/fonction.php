...
?>
		<menu><?php 
						...
						foreach($_SESSION['sql']['sousmenus'] as $k=>$v)
								...
								{		echo utf8_encode("<li><a class='".$un[$k-2]."' href='index.php?menu=".$v['url']."'>".$v['menu']."</a></li>");
	...
	?>
		<form action='index.php?cle=0' method='post' id='retraite'>
				<fieldset>
						...
						<input style="width:140px;" type='date' name='date2' id='date2' title="Date de départ souhaité" onchange='if(retraite.date1.value=="")alert("Entrez une date d arrivée");if((new Date(this.value)-1)<(new Date(preSib(this,"input").value)-1))alert(Entrez une date d arrivée valide);'/><span style="float:right">Date de départ :</span>
						<?php } ?>
				...
				<?php if($_GET['menu']!="location-espace-de-priere-abidjan"){ ?><a onclick="retraite_a(this)" id='payer' title='Passer au paiement de votre inscription'>Passer au paiement</a><?php } ?>
		</form>
		<?php }
...
foreach($_SESSION['sql']['sql0'] as $k=>$v)
		if($_GET['menu']==$v['url'])
				$Bh2[]=utf8_encode("<span class='fhid'>Esprit-Saint</span>".$v['keyword']);
...
// );
$video=array('ecommerce-religieux-chretien');
$video=$video[0];
// $video=$video[rand(0,(count($Bh1)-1))];
if(isset($_GET['menu']))switch($_GET['menu']){
...
bal(array('img','images/ecommerce-chretien-catholique-puissance-saint-esprit.png'),array('alt'=>"Retrouvez nos retraites spirituelles à Abidjan au coeur d'accueil de Jésus de Bolobi",'title'=>"retraite de prière grégorienne, à l'esprit-saint, recollection etc.."));
bal(array('video',"videos/$video.mp4",array('controls'=>'controls')),array("Retrouvez nos retraites spirituelles à Abidjan au coeur d'accueil de Jésus de Bolobi","retraite de prière grégorienne, à l'esprit-saint, recollection etc.."),0);
...
?>
				<menu><?php 
						...
						foreach($_SESSION['sql']['sousmenus'] as $k=>$v)
								...
								{		echo utf8_encode("<li><a class='".$un[$k]."' href='index.php?menu=".$v['url']."'>".$v['menu']."</a></li>");
										// PRINT_r($_SESSION['sql']['menu']);
								}
				?></menu>
<?php 			if($x!=''){ ?>
