<style>#containall h2{text-align:center;border-bottom:solid;}</style>
<h1 style='text-align:center;font-size:2em;top:-1em;width:87%;'>Panier ecommerce librairie puissance divine</h1>
<?php 
...
$tmp='';
		if(isset($_POST['envoie2'])){
				echo"	<h2>Etape2: Saisissez vos informations personnelles</h2>
						<form action='".$_SERVER['PHP_SELF']."' method='post'>
								<input type='submit' name='envoie' value='Retour étape 1'/>
						<hr/>
						</form>";
				include('formulaire_commande.php');
		}elseif(isset($_POST['envoie3'])){echo"	<h2>Etape3: Effectuer le paiement de votre commande</h2>
				...
				if(count($_COOKIE['panier'])){
						...
						echo"<h2>Etape1: Confirmez votre commande</h2>";
						$tmp='';
						...
						<form action='index.php' method='POST' onsubmit='paniersub(this)'>
						...
						?>
								...
								<?php if($promo!=0)echo"<span>réduction de ".money($promo)." avec code réduction '".$_SESSION['pdapromo']['id']."' </span>"; ?>
								<hr/>
