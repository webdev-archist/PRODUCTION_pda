...
}(document, 'script', 'facebook-jssdk'));</script>			<div class="fb-share-button" data-href="https://developers.facebook.com/docs/plugins/" data-layout="button"></div></li>
			<li>			</li>
		</ul>
<div id='enseignement'>
<?php
...
if($_GET['menu']=='archive_enseignements'){$y=date('Y');$w=date('W');
		...
		if(!$result=$mysqli->query($rqt))echo$rqt;
				...
				while($v=$result->fetch_array())
						...
						<a class='archive' href="index.php?menu=archive_enseignement&index=<?php echo $v['user_name']; ?>">
								<div>Enseignement du <?php echo trouver_date($v['user_name'],$v['img'],0); ?></div>
		...
		else{	if($result=$mysqli->query("select count(user_name) as a from blog where user_name<1000;"))if(mysql_num_rows($result)==0)$nbr=$result->fetch_array();$nbr=$nbr['a'];
		...
		<h2><?php echo$v['titre']; ?></h2><?php if($_GET['menu']=='enseignements'){ ?>
		...
		<a href='enseignements/index.php?enseignements-spirituels=<?php echo$v['phrase']; ?>' title="permalien, enseignement spirituel, saint-esprit, librairie puissance divine d'amour" style='' target='_blank'>permalien</a>
				<?php
						// utf8_encode(include("content/ens/$fb.htm"));
						echo"<iframe src='content/enseignements-spirituels/$fb.htm' id='date_enseignement' ></iframe>";
