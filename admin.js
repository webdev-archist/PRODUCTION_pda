...
function ajaxme(fonction,fichierphp,requete,settins){
	// alert(is_array(settins));
	
	if(typeof(settins)=='undefined'){var method="POST";settin1="Content-Type";settin2="application/x-www-form-urlencoded";}
	else{if(typeof(settins['method'])=='undefined')var method="POST";else method=settins['method'];
		if(typeof(settins['setting'])!='undefined')
			if(is_array(settins['setting'])){var settin1=settins['setting'][0];var settin2=settins['setting'][1];}
			else{settin1="Content-Type";settin2="application/x-www-form-urlencoded";}
		else{settin1="Content-Type";settin2="application/x-www-form-urlencoded";}
	}
		...
		var tmp="";
		for(a in requete)tmp+=a+"="+requete[a]+"&";
		tmp=tmp.substring(0,tmp.length-1);
		// alert(tmp);
			var xhr = new XMLHttpRequest(); 
				xhr.onreadystatechange = function(bi) {
					if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
						// toast(xhr.responseText); // C'est bon \o/
						fonction(xhr);
						// bi(xhr.responseText); // C'est bon \o/
					}
				};
...
function addHtml(x,y,z,zz,xy){if(typeof y=='string')y=document.createElement(y);if(typeof x=='string')document.getElementById(x).appendChild(y);else x.appendChild(y);if(typeof z=='undefined')return y;else if(z!=''&&z!='*'){addText(y,z);}if(typeof zz!='undefined'){if(zz!='')addAtt(y,zz);if(typeof xy!='undefined')return y;}}
