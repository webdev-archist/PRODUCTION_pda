<?php
		...
		// var_dump($_FILES);
		// var_dump($_POST);
		if(isset($_FILES['diapp0'])){$i=0;
				// var_dump($_FILES);
				while(isset($_FILES["diapp$i"])){
						if($_FILES["diapp$i"]['name']!='')
						{		require('config.php');
								require('class/Image.php');
								$image=new Image();
								if(!$imageupl=$image->uploadzz($_FILES["diapp$i"],$_POST["diap$i"]))echo$image->msg_err;
								// var_dump($image);
						} else{}
						$i++;
				}
...
<body onload='admin()'>
		<header>
			...
			<menu>
					...
					<li>Modifiation<ul><li><a href='index.php?admin=modval_ret'>Valider une réservation</a></li><li><a href='index.php?admin=modval_cmd'>Valider une vente en ligne</a><li class='pond'></li></li><li><a href='index.php?admin=modarr'>Modifier le stock d'un produit</a></li><li><a href='index.php?admin=modactiv'>Activer/désactiver un produit</a></li><li class='pond'></li><li><a href='index.php?admin=modsupp'>Supprimer un produit</a></li><li class='pond'></li><li><a href='index.php?admin=moddiap'>Actualiser diaporama accueil</a></li></ul></li>
					<li><a class="admin_menu" href='index.php?admin=compta'>Inventaire des ventes</a><ul><li><a href='index.php?admin=quot'>Quotidien</a></li><li><a href='index.php?admin=hebdo'>Hebdomadaire</a></li><li><a href='index.php?admin=mens'>Mensuel</a></li></ul>
					<li><a class="admin_menu" href='index.php?admin=posts'>Publier sur les Réseau Sociaux</a></li>
					<li><a class="admin_menu" href='index.php?admin=assist'>Assistance spirituelle</a></li>
...
<?php 
		if(isset($_GET['admin']))
				...
				<form action='index.php' method='post' class='modifier <?php //if($_GET['admin']=='moddiap')echo'cov'; ?>' id='modifierr' enctype="multipart/form-data">
				<?php
						switch($_GET['admin']){
								...
								<?php break;
								case'modsupp':$x='supprimer';?><input type='hidden' id='article_mod' name='article_mod' value='<?php echo$x; ?>'/><input type='hidden' id='id' name='id' value=''/>
										<fieldset><legend>Entrer le <b>nom</b> de l'article à supprimer, <b>cliquez sur</b> ce dernier dans la liste des choix, <b>PUIS VALIDER!</b></legend><input type='text' value='' name='mod' id='mod' onkeyup="modifier('<?php echo$x; ?>',this)"/> </fieldset>
										<fieldset><legend></legend></fieldset>
										
								<?php break;
								case'moddiap':$x='diaporama';?><input type='hidden' id='article_mod' name='article_mod' value='<?php echo$x; ?>'/><input type='hidden' id='id' name='id' value=''/>
								<?php 	
										$mysqli=$_SESSION['mysqli'];
										// $mysqli=new mysqli('localhost','pda','mirai','admin_pda');
										// $mysqli=new mysqli('localhost','root_pda_website','q3p_Vv75','admin_pda');$mysqli->set_Charset("utf8");$cat=array();
										if($result=$mysqli->query("select distinct id from diapo"))while($v=$result->fetch_array())$cat[]=$v['id'];
										if($result=$mysqli->query("select * from diapo"))while($v=$result->fetch_array())$cat_img[]=$v;
										echo"";foreach($cat as $v){}
								?>
										<fieldset><legend>Selectionnez la brève que vous souhaitez modifier</legend><select name='diap' id='diap' onchange="modifier('<?php echo$x; ?>',this)"/><option value=''>Selectionnez une action</option><option value='new'>Ajouter un nouveau thème</option><?php foreach($cat as $k=>$v){echo"<option value=\"".str_replace(' ','#_#',$v)."\">$v</option>";} ?></select> </fieldset>
										<div id='diap_cont'></div>
										<input type='hidden' value='' name='id' id='id' />
										<input type='hidden' value='' name='img' id='img' />
										<input type='hidden' value='' name='breve' id='breve' />
										<input name='diap0' id='diap0' type='hidden'/>
										<input name='diap1' id='diap1' type='hidden'/>
										<input name='diap2' id='diap2' type='hidden'/>
										<input name='diap3' id='diap3' type='hidden'/>
										<input type="file" name='diapp0' id='diapp0' class='hid' onchange="diapoo(this,0)" />
										<input type="file" name='diapp1' id='diapp1' class='hid' onchange="diapoo(this,1)" />
										<input type="file" name='diapp2' id='diapp2' class='hid' onchange="diapoo(this,2)" />
										<input type="file" name='diapp3' id='diapp3' class='hid' onchange="diapoo(this,3)" />
										<div id='complement' class='hid'>
												<h3 class='img' contentEditable onmousedown="getClass(this,'see');" onblur="gid('diap'+envoies.param).value=gid('diap'+envoies.param).value.substring(0,gid('diap'+envoies.param).value.lastIndexOf('/')+1)+this.innerHTML+document.getElementById('diap'+envoies.param).value.substring(document.getElementById('diap'+envoies.param).value.length-4);"></h3>
												<img src='' title="L'image que vous avez choisi pour la brève en cours" />
												<div id='alt' onblur="gid('diap'+envoies.param).dataset.alt=this.innerHTML" contentEditable>Entrez votre attribut alt</div>
												<div id='texte' onblur="gid('diap'+envoies.param).dataset.texte=this.innerHTML" contentEditable>Entrez votre texte</div>
												<input type='button' value='push' onclick="if(EV(clas('hid',this.form)[envoies.param].src))toast(clas('hid',this.form)[envoies.param].src);if(EV(clas('hid',this.form)[envoies.param].texte))toast(clas('hid',this.form)[envoies.param].texte);if(EV(clas('hid',this.form)[envoies.param].alt))toast(clas('hid',this.form)[envoies.param].alt);" name='z'/>
										</div>
										
								<?php break;
								default:break;
						}?>
						<input type='submit' value='Envoyer' id='envoie<?php if($_POST['admin']=='moddiap')echo"s";?>' name='envoie<?php if($_POST['admin']=='moddiap')echo"s";?>'/>
				</form>
				...
				<td title="<?php echo giveDateFr($vv['heures'],'abrevEn','abrevEn')." (function à faire pour l'écriture complète de la date en français)"; ?>"><?php echo giveDateFr($vv['heures'],'abrevEn','abrevEn'); ?></td>
				</tr>
				<?php }}  ?>
				</tbody>
				</table>
				<?php } elseif($_GET['admin']=='assist'){ 

						$mysqli=$_SESSION['mysqli'];
						// $mysqli=new mysqli('localhost','pda','mirai','admin_pda');
						// $mysqli=new mysqli('localhost','root_pda_website','q3p_Vv75','admin_pda');
						$mysqli->set_Charset("utf8");
						$rqt="select * from newsletter where id='in';";
						echo"Les différentes demande d'intercession:<br/>";
						if($result=$mysqli->query($rqt))
								while($v=$result->fetch_array())
										echo"<div><span>".$v['email']."</span>".$v['nom']."</div>";
						else echo"<span style='color:red'>aucune demande de prière n'a encore été faite</span>";
				?>
						
				<?php } elseif($_GET['admin']=='posts'){ ?>
				<form id='rs_admin'>
						<h3>Choisir :</h3>
						<button id='firstb' type='button' onclick='return nl("");'>Envoyer newsletter par email</button>
						<br/>
						<fieldset>
								<legend>Diffuser sur les réseaux sociaux</legend>
								<select id='newsletter' onchange='nl("","lien")'>
										<option selected value=''>Selectionnez une newsletter</option>
										<option value='en'>Enseignement spirituel hebdomadaire</option>
										<option value='pe'>Pensée quotidienne</option>
										<option value='pr'>Prière Mensuelle</option>
										<option value='ac'>Actualité LPDA</option>
								</select>
						</fieldset>
						<!--<button onclick='toast(nexSib(this,"div").content);nexSib(this,"div").innerHTML=nl_emails.value;return false'>Voir le(s) lien(s)</button><div></div>-->
						<textarea name="nl_emails" id="nl_emails"></textarea><div id="nl_emails_response"></div>
						<button onclick='nl("","lien")'>Envoyer sur Facebook !</button>
						<button onclick='nl("","lien")'>Envoyer sur Twitter !</button>
						<button onclick='nl("","lien")'>Envoyer sur Google+ !</button>
				...
				if(isset($_POST['envoie_vente'])||isset($_POST['envoie_retraite'])){ //include('process.php'); 
						...
						if(isset($_POST['id_produits'])){
								...
								else	echo"<br/>#_#".$image->msg_err."<br/>";
								...
								if($_POST['couverture']!='')$rqtbis="INSERT INTO `options_article` (`id`, `img_article`, `stock`, `vendus`, `couverture`)values('','".$_POST['img']."','10','0','".$_POST['couverture']."');";else {}
						...
						if(isset($_POST['type'])){echo"identifiant de l'opération: ".$_POST['id']."<br/>";
								...
								if($result=$mysqli->query($rqt)){
										...
										echo"Validation de la ".$_POST['type']." confirmée avec les requete:<br/>
										$rqt<br/>update ret set places=places-".$_POST['nombre']." where id='".$_POST['id_retraite']."';
										<br/><br/>";
										
								}else echo$rqt;
						}else{
								if($_POST['article_mod']=='supprimer'){if(!$result=$mysqli->query("delete from articles where id_produits=".$_POST['id'].";"))echo"delete from articles where id_produits='".$_POST['id']."';";else echo"suppression réussie !";
								}elseif($_POST['article_mod']=='stock')	
