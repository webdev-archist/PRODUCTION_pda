...
function init(){window.onscroll=function(){if(typeof(articles)!='undefined'){if(getScrollPosition()[1]>289)addClass(paniers,'pdown');else paniers.removeAttribute('class');}};
		...
		if(typeof(articles)!='undefined'){artcles=new Array("","icone","divers","encens","croixm","croixp","chapelet","grottes","statues")
				tag('div',affiche_article)[0].addEventListener('click',function(e){Event.stop(e);},false);
				for(var i=0;i<clas('panier').length;i++)
						clas('panier')[i].onclick=function(x){addarticle(this,x);}
		...
		if(location.href.indexOf('#')!=-1){
								// alert('ok');
		//fonction #tag du select dans vente-en-ligne 
		//A FAIRE! ! !
				if(artcles.indexOf(location.href.substring(location.href.indexOf('#')+1))!=-1){}
		//----
				for(var i=0;i<tag('a',tag('footer')[0]).length;i++)
...
// alert(y+x.selectedIndex);
// alert(x.selectedIndex);
// alert(tag('option',x)[x.selectedIndex]);
		location.hash = "#"+artcles[x.selectedIndex];
...
function addarticle(x,y){		
								...
								// alert(preSib(x).max);
								if(preSib(x).value!=0&&preSib(x).value<=parseInt(preSib(x).max)){
										var date=new Date();
										date.setTime(date.getTime()+(5*24*3600*1000));
										var expire=";expires="+date.toGMTString();
										if(x.parentNode.previousSibling.previousSibling.innerHTML.indexOf('|')!=-1)
												document.cookie="panier["+x.parentNode.previousSibling.previousSibling.innerHTML+"]="+preSib(x).value+expire+"; path=/";
										else	document.cookie="panier["+parseInt(x.parentNode.previousSibling.previousSibling.innerHTML)+"]="+preSib(x).value+expire+"; path=/";
										...
										if(clas('articles').length!=0){}
										else if(y.clientY==0){
												// getCss(le_panier,'display:block;top:'+tag('header')[0].offsetHeight+'px;');
												getCss(le_panier,'display:block;top:'+(x.offsetParent.offsetTop+y.clientY)+'px;');
										}else
												getCss(le_panier,'display:block;top:'+y.clientY+'px;');
												getCss(le_panier,'display:block;top:'+(x.offsetParent.offsetTop+y.clientY)+'px;');
										// setTimeout("le_panier.style.opacity=0;",4000);
										// aa('star');
										setTimeout("le_panier.removeAttribute('style');",9000);
										// alert(le_panier.parentNode);
												resetClass(paniers,'plus1');
												// alert(x.offsetParent.offsetTop);
												// alert(document.body.clientHeight);
												cpt=addHtmll(tag('body')[0],'img','','src==images/plus1.png!!style==border-radius:100px;position:fixed;top:'+y.clientY+'px;right:'+(document.body.clientWidth-y.clientX)+'px;!!class==transitionall3 shine');
												cptbis=x.offsetParent.offsetTop;
												setTimeout("addCss(cpt,'top:-"+cptbis+"px;right:50px;');",700);
												setTimeout("tag('body')[0].removeChild(tag('body')[0].lastChild);;",2200);
												// alert(y);
												// alert(y.clientY);
												// alert(x.offsetHeight+'px;');
								}else{
										// alert(ic+"Veuillez entrer une quantité à ajouter au panier:\n\n'"+clas('l',x.parentNode.parentNode)[0].innerHTML+"'");
										toast("Veuillez entrer une quantité (à l'aide des flèche) de:<br/>"+clas('l',x.parentNode.parentNode)[0].innerHTML+"'\n\n à ajouter au panier");
								}
}
...
function retraite_a(x,y){var alertt='';
		// alert(retraite.param);
		// if(montant.innerHTML==''){
				...
				if(tag('h2',retraite_descr)[0].innerHTML.indexOf('eek')!=-1){getText(montant,money(14000*nombre.value));
						// alert(week.date.value+"\n"+tag('input',dates).length);
						retraite.montant.value=14000*nombre.value;
				...
				// else if(tag('h2',retraite_descr)[0].innerHTML.indexOf('ocatio')!=-1)getText(montant,money(10000*nombre.value));
				else if(tag('h2',retraite_descr)[0].innerHTML.indexOf('ocatio')!=-1)if(y-x<0)alertt+='Veuillez choisir des dates valides.\n\n';else {alert(1000*nombre.value);alert(diffJours(x,y).day);alert(1000*nombre.value*diffJours(x,y).day);retraite.montant.value=1000*nombre.value*diffJours(x,y).day;location_info.innerHTML=money(retraite.montant.value);}
		...
		if(alertt!='')alert(alertt);
				...
				if(x.id=='payer'){
						...
						if(tag('h2',retraite_descr)[0].innerHTML.indexOf('ocatio')==-1)unsee(payer);
						unsee(reserve);
...
function ret(x){//alert(clas('ret_info')[0].innerHTML);
		if(x=='ouverte'){see(clas('ret_info')[0]);getText(montant,money(10000));}
		else if(x=='fermee') {see(clas('ret_info')[1]);getText(montant,money(50000));}
		else getText(montant,money(50000));
...
function seeee(x){if(clas('see',document.getElementById(x).parentNode).length!=0)if(typeof(y)=='undefined')clas('see',document.getElementById(x).parentNode)[0].removeAttribute('class');else getClass(clas('see',x.parentNode)[0],y);addClass(x,'see');}
...
function nl(x,y){//alert(x);
		if(x!=''){//alert(x.responseText);
				if(x.responseText.indexOf("iste des emails env")!=-1){
						responsetext.innerHTML=x.responseText;
				}else if(x.responseText.indexOf("ivez la newsletter reli")!=-1){
						nl_emails.innerHTML=x.responseText;
						// toast('rien');
				}
				 toast(x.responseText);
				// nl_emails.setAttribute("ContentEditable",'true');
		}else if(typeof(y)!='undefined'){
				 // toast(newsletter);
				// toast(newsletter.selectedIndex);
				// toast(newsletter.value);
				// firstb.removeAttribute('disabled');
				switch(y){
						case"lien":ajaxme(function(x){nl(x);}						,"ajaxmeee.php"						,{"sh_nl":newsletter.value});break;
				default:break;
				}
		}else{alert(newsletter.value);
				// firstb.setAttribute('disabled','true');
				ajaxme(function(x){nl(x);}
						,"ajaxmeee0.php"
						,{"nl":newsletter.value});
				return false;
		}
...
function admin_ssmenu(x){//alert(tag('option',x)[x.selectedIndex].innerHTML);
		...
		if(!inArray(x.value,new Array('fermee','ouverte',''))){
				...
				getText(document.getElementById('ajouter_'+ssmenu.param),tag('input',document.getElementById(ssmenu.param))[0].parentNode.innerHTML);
...
function eraseBreve(x){
		if(prompt("Pressez 'OK' pour supprimer, 'Annuler' sinon","Cliquez 'OK' pour supprimer, 'Annuler' sinon")!=null)
				ajaxxme(new Array('onSuccess',function(x){if(x.responseText){getClass(closedd,'hid');getClass(modifierr,'modifier cov');diap_cont.innerHTML='';toast("Brève supprimé avec succès!");var cc=diap.selectedIndex;tag('option',diap)[0].click();diap.removeChild(tag('option',diap)[cc]);}else toast("ECHEC lors de la suppression de la brève !");}),'lpd.php','diaporama=erase&id='+x);
}
function save_diap(i){var cc=false;toast(tmpbis);if(typeof(i)=='undefined'){i=0;} if(i==0){modifierr.action='index.php';alert("modifierr.action="+modifierr.action);}
// alert("i="+i);
		if(i<4){while((gid('diap'+i).value==''))if(i<3){i++;}else {i=0;var cc=true;}
				if(cc)i=-2;
				if(tmpbis==-1){
						ajaxxme(new Array('onSuccess',function(x){if(x.responseText>0)save_diap(x.responseText)}),'lpd.php','diaporama=insert&img='+gid('diap'+i).value+'&nom='+diap.value.replace(/#_#/g,' ')+'&alt='+gid('diap'+i).dataset.alt+'&texte='+gid('diap'+i).dataset.texte.replace(/'/g,'#acute;')+'&i='+i+'&id_='+gid('diap'+i).dataset.id_);
				}else if(i>-1){//alert('diaporama=insert&img='+gid('diap'+i).value+'&nom='+diap.value.replace(/#_#/g,' ')+'&alt='+gid('diap'+i).dataset.alt+'&texte='+gid('diap'+i).dataset.texte.replace(/'/g,'#acute;')+'&i='+i+'&id_='+gid('diap'+i).dataset.id_);
						ajaxxme(new Array('onSuccess',function(x){if(x.responseText>0)save_diap(x.responseText)}),'lpd.php','diaporama=update&img='+gid('diap'+i).value+'&nom='+diap.value.replace(/#_#/g,' ')+'&alt='+gid('diap'+i).dataset.alt+'&texte='+gid('diap'+i).dataset.texte.replace(/'/g,'#acute;')+'&i='+i+'&id_='+gid('diap'+i).dataset.id_);
				}//alert('tmpbis'+tmpbis);
		}
		if(i==-2){toast('envoie en cours');envoies.onmousedown=function(){};envoies.click();
		}
		// envoies.onmousedown=function(){toast('envoie!');};
}
function diapoo(x,y){envoies.param=y;//toast('envoies.param  =  '+envoies.param);
		if(complement.hasAttribute('class'))complement.removeAttribute('class');
		// removeClass(x.form,'cov');
		// alert(x.value);

		gid('diap'+envoies.param).dataset.alt=alt.innerHTML;
		gid('diap'+envoies.param).dataset.texte=texte.innerHTML;
		gid('alt').onfocus=function(){this.setAttribute('class','');};
		gid('texte').onfocus=function(){this.setAttribute('class','');};
		// gid('diap'+envoies.param).value=gid('diap'+envoies.param).value.substring(0,gid('diap'+envoies.param).value.lastIndexOf('/')+1)+img.innerHTML+gid('diap'+envoies.param).value.substring(gid('diap'+envoies.param).value.length-4);
		
		tmp=x.value;
		addClass(tag('h3',complement)[0],'see brille');
		addClass(texte,'see brille');
		addClass(alt,'see brille');
		getText(tag('h3',complement)[0],tmp.substring(tmp.lastIndexOf('\\')+1,tmp.length-4));
		img.value=tag('h3',complement)[0].innerHTML;
		x.dataset.src=tag('h3',complement)[0].innerHTML;
		
		alert(res2);
		var cc=addHtml(tag('h3',complement)[0],'div','','onmouseover==this.className="clipped";!!onmouseout==this.removeAttribute("class");',"");
		for(a in res2){if(parseInt(a)>0)addHtml(cc,'div',res2[a],"onclick==tag('h3',complement)[0].innerHTML=this.innerHTML",'');}
		// tag('img',complement)[0].src='images/'+this.value.substring(12);
		// tag('img',complement)[0].src='images/'+tag('option',diap)[diap.selectedIndex].innerHTML.toLowerCase().replace(/ /g,'-').replace(/'/g,'-')+'/'+tmp.substring(tmp.lastIndexOf('\\')+1);
		// alert(document.getElementById(x.id.replace('p','')));
		document.getElementById(x.id.replace('p','')).value='images/'+tag('option',diap)[diap.selectedIndex].innerHTML.toLowerCase().replace(/ /g,'-').replace(/'/g,'-')+'/'+tmp.substring(tmp.lastIndexOf('\\')+1);
		// alert(document.getElementById(x.id.replace('p','')).value);
}
function modifier(x,y){//alert(x+y);
		if(y.value.length>2)
				ajaxxme(new Array('onSuccess',function(x){
						// alert(article_mod.value);
						getText(choix_admin,x.responseText);
						if(article_mod.value=='diaporama'){diap_cont.innerHTML='';var h='';tmpbis=0;//alert(x.responseText);
								if(typeof(closedd)!='undefined'){if(closedd.hasAttribute('class'))closedd.removeAttribute('class');}
								// getClass(modifierr,'modifier cov cover');
								getClass(complement,'hid');
								modifierr.action='javascript:return false;';
								envoies.onmousedown=function(){alert(tag("option",diap)[diap.selectedIndex].param);if(tag("option",diap)[diap.selectedIndex].param)tmpbis=-1; save_diap()};
								choix_admin.innerHTML='';
								// toast(x.responseText);
								res2='';if(x.responseText=='') var res=new Array();
								else{ 	if(x.responseText.indexOf("#_#")==-1)var res = window.JSON.parse(x.responseText);
										else{var res = window.JSON.parse(x.responseText.substring(0,x.responseText.indexOf("#_#")));
										res2= window.JSON.parse(x.responseText.substring(x.responseText.indexOf("#_#")+3));
										}
								}alert(res2);
								for(var i=0;i<4;i++)
								{		if(typeof(res[i])=='undefined'){res[i]={'img':'','alt':'','texte':'','id':'','id_':''};h='height:100px;'}
										addHtml(diap_cont,'img','*',"src=="+res[i]['img']+"!!alt=="+res[i]['alt']+"!!title=="+res[i]['texte']+"!!data-id_=="+res[i]['id_']+"!!width==100px!!style==cursor:pointer;"+h+"!!data-i=="+i+"!!onclick==gid('alt').innerHTML=this.alt;gid('texte').innerHTML=this.title;gid('diap'+dataset.i).dataset.id_=dataset.id_;if(Boolean(this.src)){document.getElementById('diapp'+dataset.i).click();}");
								}		
								if(y.value=='new'){var aa=addHtml(diap_cont,'div','',"contenteditable==true!!class==diap_cont brille!!onmousedown==getClass(diap_cont,'diap_cont');!!onblur==tag(\"option\",diap)[diap.selectedIndex].param=\"true\";alert(tag(\"option\",diap)[diap.selectedIndex].param);tag(\"option\",diap)[diap.selectedIndex].value=this.innerHTML.replace(/ /g,\"#_#\");tag(\"option\",diap)[diap.selectedIndex].innerHTML=this.innerHTML;this.parentNode.removeChild(this);toast(\"si vous n'avez pas bien entré le nom de votre brève, veuillez actualiser la page\");");}
								// if(typeof(erase)=='undefined')addHtml(tag('fieldset',modifierr)[0],'span','X',"title==supprimer cette brève ?!!id==erase!!onclick==eraseBreve(diap.value.replace(/#_#/g,' '))");
								if(typeof(closedd)=='undefined')addHtml(tag('fieldset',modifierr)[0],'span','X',"title==supprimer cette brève ?!!id==closedd!!onclick==eraseBreve(diap.value.replace(/#_#/g,' '))");
						}else 
						for(var i=0;i<tag('div',choix_admin).length;i++)
								if(article_mod.value=='supprimer'){alert(id.value);
								}else tag('div',choix_admin)[i].onclick=function(){
										id.value=tag('span',this)[0].innerHTML;
										mod.value=skip_bal(tag('span',this)[1].innerHTML);
				...
				}),'lpd.php','admin_='+x+'&valeur='+y.value+"&lang="+langue);
		// else if(typeof(closedd)!='undefined'){getClass(closedd,'hid');diap_cont.innerHTML='';getClass(modifierr,'modifier cov');}
		else if(typeof(closedd)!='undefined'){getClass(closedd,'hid');diap_cont.innerHTML='';}
}
