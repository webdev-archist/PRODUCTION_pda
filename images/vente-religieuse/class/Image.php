...
public $msg_error='';
	...
	private function connexion(){
		$this->mysqli=new mysqli('localhost','root','','gest_img');
	...
	public function uploadzz($v,$y){//echo"lkjkljkljk";
		// var_dump($v);
		// var_dump($y);
		
		// $upload_dir="images/";
		$upl_dir=IMAGES_UPLOADEDZ;
		// $upload_dir=";
		$err=0;
		$this->msg_err="Upload réussi";
		if($err==UPLOAD_ERR_OK)
				if($v["type"]=='image/jpeg'||$v["type"]=='image/png'){
						if($v["type"]=='image/jpeg')$ext='jpg';else $ext='png';
						$tmp=$v["tmp_name"];
						if(!isset($_POST['img']))$name=$v["name"];else $name=$_POST['img'].'.'.$ext;
						if(!move_uploaded_file($tmp,$upl_dir.$name)){$err++;$this->msg_err="Erreur lors de l'upload, le fichier '$name' n'a pas pu etre déplacé à: $upl_dir.$name";}
						if(!copy($upl_dir.$name,$y)) $this->msg_err.= "La copie $upload_dir$name du fichier a échoué...\n";
						// $this->creer_vignette($upload_dir.$y.$name,$y.'/min/qddsqsqd.jpg');
				}else{$this->msg_err="Type de fichier uploadé incorrecte: image/jpeg<br/>Entré: ".$v["type"];}	
		else{$this->msg_err='Erreur, veuillez réessayer.';}
		// echo"oooo";
		echo$y."|||";
		// echo$upl_dir."|||";
		// echo$upl_dir.$name."|||";
		// echo$upload_dir.$y.$name;
		...
		foreach($files['upl']['error'] as $k=>$v)://print_r($v);
			...
			if($err==UPLOAD_ERR_OK)
				if($files['upl']["type"][$k]=='image/jpeg'||$files['upl']["type"][$k]=='image/png'){
					...
					if(!move_uploaded_file($tmp,$upload_dir.$y.$name)){$err++;$this->msg_err="Erreur lors de l'upload, le fichier '$name' n'a pas pu etre déplacé à: $upload_dir";}
					if(!copy($upload_dir.$y.$name,$upl_dir.$name)) echo "La copie du fichier $upload_dir$y$name a la destination $upl_dir.$name a échoué...\n";
					// if(!copy($upload_dir.$y.$name,$upload_dir.$y.substr($name,0,count($name)-5).'-cov.'.$ext)) echo "La copie du fichier $upload_dir$y$name a échoué...\n";
