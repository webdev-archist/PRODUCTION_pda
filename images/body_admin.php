		...
		if(isset($_FILES['diapp0'])){$i=0;
				...
				while(isset($_FILES["diapp$i"])){
						...
						{		require_once('images/vente-religieuse/index.php');
								$image=new Image();
								if(!$imageupl=$image->uploadzz($_FILES["diapp$i"],$_POST["diap$i"]))echo"nooooooooon";
						} else{}
						$i++;
				}
		}else {echo"kljlkj";}
...
<?php 
		if(isset($_GET['admin']))
				...
				<form action='index.php' method='post' class='modifier' id='modifierr' enctype="multipart/form-data">
				<?php
						switch($_GET['admin']){
								...
								?>
										...
										<div id='complement'>
												<h3 class='img' contentEditable onmousedown="getClass(this,'see');" onblur="clas('hid',this.form)[envoie.param].dataset.src=this.innerHTML;"></h3>
												<img src='' title="L'image que vous avez choisi pour la brève en cours" />
												<div id='alt' onblur="clas('hid',this.form)[envoie.param].dataset.alt=this.innerHTML" contentEditable>Entrez votre attribut alt</div>
												<div id='texte' onblur="clas('hid',this.form)[envoie.param].dataset.texte=this.innerHTML" contentEditable>Entrez votre texte</div>
												<input type='button' value='push' onclick="if(EV(clas('hid',this.form)[envoie.param].src))toast(clas('hid',this.form)[envoie.param].src);if(EV(clas('hid',this.form)[envoie.param].texte))toast(clas('hid',this.form)[envoie.param].texte);if(EV(clas('hid',this.form)[envoie.param].alt))toast(clas('hid',this.form)[envoie.param].alt);" name='z'/>
										</div>
										
								<?php break;
								default:break;
						}?>
						<input type='submit' value='Envoyer' id='envoies' name='envoies'/>
				</form>
