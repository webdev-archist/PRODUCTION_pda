...
// switch($_GET['menu']){
				...
				// $ss=explode(";",$page['img']);
				// $sss=explode(";",$page['video']);
// bal(array('img','images/'.utf8_encode($ss[0].'.png')),array("alt"=>utf8_encode($ss[1]),"title"=>utf8_encode($ss[2])));
// bal(array('video',$sss[0],array("controls"=>"controls","width"=>"200")),array("title"=>utf8_encode($sss[1]),utf8_encode($sss[2])));

?>
		<menu><?php 
						foreach($_SESSION['sql']['sousmenus'] as $k=>$v){
								if($_GET['menu']==$v['url'])$kk="class=ssmenu_hover";else $kk='';
								if($v['user_name']=='priere')
								{		echo utf8_encode("<li><a $kk href='index.php?menu=".$v['url']."' title='".$v['descr']."'>".$v['menu']."</a></li>");
...
<?php
		...
		function vente($x,$y=''){$cpt=array();
				$un=array('','');
				if($y=='')$page=$x;else{$page=$y;if($x=="objet")$un[1]='ssmenu_hover';else$un[0]='ssmenu_hover';}
				// echo $x;
				...
				// $mysqli=new mysqli('localhost','root_pda_website','oEc7x5?7','admin_pda');
				$mysqli->set_Charset("utf8");
				if(gettype($x)=="string")$rqt='select id_produits,auteur,user_name,nom,img,fr,fr1,autre,taille,prix,stock,vendus,actif from articles where user_name="'.$x.'" and actif="1" order by id_produits ASC;';
...
$Bh2=array();
$Bh2[]=utf8_encode("<span class='fhid'>Esprit-Saint</span>".$page['keyword']);// $video=array('ecommerce-religieux-chretien');$video=$video[rand(0,(count($Bh1)-1))];
$video=explode(';',$page['video']);$img=explode(';',$page['img']);
...
// bal('h2',utf8_encode($page['textH']));
// bal('h3',utf8_encode($page['keyword']));
// bal(array('img','images/'.$img[0].'.png'),array('alt'=>utf8_encode($img[1]),'title'=>utf8_encode($img[2])));
// bal(array('video',"videos/".$video[0].".mp4",array('controls'=>'controls')),array(utf8_encode($video[1]),utf8_encode($video[2])),0);
...
?>
				<menu><?php 
						...
						foreach($_SESSION['sql']['sousmenus'] as $k=>$v)
								...
								// {		echo utf8_encode("<li><a class='".$un[$k]."' href='index.php?menu=".$v['url']."' title='".$v['descr']."'>".$v['menu']."</a></li>");
								{		echo utf8_encode("<li><a class='__\$un[$k]' href='index.php?menu=".$v['url']."' title='".$v['descr']."'>".$v['menu']."</a></li>");
										// PRINT_r($_SESSION['sql']['menu']);
								}
				?></menu>
<?php 			if($y!=''){ ?>
...
<?php							$options_article=array();
								...
								if($result=$mysqli->query($rqt))
										while($v=$result->fetch_array()){$dispo='';
												...
												?>
														<tr <?php if($v['nom']=='grotte')echo'class="'.$v['nom'].'"'; ?>>
																...
																<td class='l'><?php echo$v['fr'];?><a href='http://librairie-puissance-divine.ci/vente-en-ligne/index.php?produit-religieux=<?php echo$v['id_produits']; ?>' title='ecommerce religieux article chrétien: <?php echo rmv_balise_(substr($v['fr'],0,30)); ?>' target='_blank'>voir ce produit <?php echo$libelle[$v['nom']]; ?></a></td>
										...
										<?php 	}else {
											echo$rqt."<br/>lkjkljklkjjklj";
											var_dump(mysqli_error($mysqli));
											printf("Message d'erreur0 : %s\n", $mysqli->error);
											printf("Message d'erreur1 : %s\n", mysqli_error($mysqli));
										} ?>
										</tbody>
								</table>
...
<?php
		...
		<?php }
		function lieux($x=''){
if(isset($_GET))switch($_GET['menu']){case'puissance-divine-fraternite':$un=array('ssmenu_hover','','');break;case'puissance-divine-librairie':$un=array('','ssmenu_hover','');break;case'puissance-divine-sanctuaire':$un=array('','','ssmenu_hover');break;default:$un=array('','','');break;}
		?>
				<menu><?php 
						// var_dump($_SESSION['sql']['sousmenus']);
						// echo$_SESSION['sql']['sousmenus'][6];
						foreach($_SESSION['sql']['sousmenus'] as $k=>$v)
								if($v['user_name']=='objectifs')
								{		echo utf8_encode("<li><a class='".$un[$k]."' href='index.php?menu=".$v['url']."' title='".$v['descr']."'>".$v['menu']."</a></li>");
										// PRINT_r($_SESSION['sql']['menu']);
										// PRINT_r($un);
										// echo $k-6;
